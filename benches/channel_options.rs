        ...
        let handle = thread::spawn(move || {
            loop {
                match *recv_func.recv().expect("AHH") {
                    ...
                    WorkerFuncs::GetA() => send_ret
                        .send(Box::new(WorkerReturns::GetA(thingy.get_a())))
                        .unwrap(),
                    WorkerFuncs::SetAndGetA(snd, i) => {
                        send_ret
                            .send(Box::new(WorkerReturns::SetAndGetA(thingy.set_and_get_a(i))))
                            .unwrap()
                        //snd.send(thingy.set_and_get_a(i)).unwrap()
                    }
                    WorkerFuncs::SetAndGetUnsafe(i) => send_ret
                        .send(Box::new(WorkerReturns::SetAndGetUnsafe(
                            thingy.set_and_get_b(i),
                        )))
                        .unwrap(),
...
impl ThingyController {
    ...
    pub fn assert_false(&self) {
        self.send
            .send(Box::new(WorkerFuncs::AssertFalse()))
            .unwrap();
    }
    pub fn plus_one_a(&self) {
    ...
    pub fn set_and_get_a(&self, i: i32) -> i32 {
        ...
        self.send
            .send(Box::new(WorkerFuncs::SetAndGetA(snd, i)))
            .unwrap();
        //rcv.recv().unwrap()
        match *self.recv.recv().unwrap() {
    ...
    pub fn set_and_get_unsafe(&self, i: i32) -> i32 {
        self.send
            .send(Box::new(WorkerFuncs::SetAndGetUnsafe(i)))
            .unwrap();
        match *self.recv.recv().unwrap() {
    ...
    pub fn set_and_get_new_channel(&self, i: i32) -> i32 {
        ...
        self.send
            .send(Box::new(WorkerFuncs::SetAndGetNewChannel(snd, i)))
            .unwrap();
        *rcv.recv().unwrap()
    }
    pub fn set_and_get_new_oneshot_channel_futures(&self, i: i32) -> i32 {
        ...
        self.send
            .send(Box::new(WorkerFuncs::SetAndGetNewOneshotChannelFutures(
                snd, i,
            )))
            .unwrap();
        match futures::executor::block_on(async move { rcv.await }) {
            Ok(x) => *x,
            Err(_) => panic!("AHHH"),
        }
    }
    pub fn set_and_get_new_oneshot_channel_tokio(&self, i: i32) -> i32 {
        ...
        self.send
            .send(Box::new(WorkerFuncs::SetAndGetNewOneshotChannelTokio(
                snd, i,
            )))
            .unwrap();

        self.rt.block_on(async { *rcv.await.unwrap() })
    }
}
